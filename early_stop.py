...
class EarlyStopping:
    def __init__(
        ...
        for stop_varname in stop_varnames:
            if stop_varname is StopVariable.LOSS:
                ...
                self.comp_ops.append(operator.le)  # 小于等于
                self.best_vals.append(np.inf)      # 正无穷
            elif stop_varname is StopVariable.ACCURACY:
                self.stop_vars.append('acc')
                self.comp_ops.append(operator.ge)   # 大于等于
                self.best_vals.append(-np.inf)      # 负无穷
    ...
    def check(self, values: List[np.floating], epoch: int) -> bool:
        ...
        if any(checks):  # 如果有一个为true，则为true， 全部为false时才会为false
            ...
            if self.remember is Best.ALL:
                if all(comp_remembered):  # all函数只有iter全部为true时返回true
